...
def fake_quantize(
    ...
    # signed quantized
    max_q = torch.tensor(2 ** int(args.num_bits // 2) - 1, device=x.device)
    min_q = -torch.tensor(2 ** int(args.num_bits // 2), device=x.device)

    Q = quantize(x, scale, zero_point, max_q, min_q)
